...
// @ts-nocheck
import { rand, randIn, TAU, PropsArray, lerp, clamp, fadeInOut } from './lvrjwz';
const { PI, cos, sin, tan, abs, sqrt, pow, min, max, ceil, floor, round, random, atan2 } = Math;
import SimplexNoise from 'simplex-noise';

const particleCount = 1000;
const particleProps = ['x', 'y', 'z', 'vx', 'vy', 'vz', 'life', 'ttl', 'speed'];
...
function drawParticle(x, y, z, x2, y2, life, ttl, width, hue) {
  ...
  ctx.a.strokeStyle = `hsla(${hue + clamp(z, 0, 180)},${clamp(z, 10, 60)}%,${clamp(z, 10, 50)}%,${
    fadeInOut(life, ttl) * (0.01 * clamp(z, 50, 100))
  })`;
  ctx.a.beginPath();
  ctx.a.moveTo(x, y);
  ctx.a.lineTo(x2, y2);
