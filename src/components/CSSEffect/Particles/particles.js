...
function launchParticlesJS(tag_id, params) {
    ...
    if (params.particles) {
      ...
      if (paramsForParticles.line_linked) {
        ...
        if (paramsForLineLinked.condensed_mode) {
          ...
          if (paramsForCondensedMode.rotateY)
            pJS.particles.line_linked.condensed_mode.rotateY = paramsForCondensedMode.rotateY;
        }
      }
    ...
    if (params.interactivity) {
      ...
      if (paramsForInteractivity.mouse) {
        if (paramsForInteractivity.mouse.distance)
          pJS.interactivity.mouse.distance = paramsForInteractivity.mouse.distance;
      }
      ...
      if (paramsForInteractivity.line_linked) {
        if (paramsForInteractivity.line_linked.opacity)
          pJS.interactivity.line_linked.opacity = paramsForInteractivity.line_linked.opacity;
      }
  ...
  pJS.fn.vendors.distanceParticles = function (p1, p2) {
    ...
    if (dist <= pJS.particles.line_linked.distance) {
      ...
      pJS.canvas.ctx.strokeStyle =
        ...
        color_line.b +
        ',' +
        (pJS.particles.line_linked.opacity - dist / pJS.particles.line_linked.distance) +
        ')';
  ...
  pJS.fn.vendors.interactivity.grabParticles = function (p1, p2) {
    ...
    if (
      dist <= pJS.particles.line_linked.distance &&
      dist_mouse <= pJS.interactivity.mouse.distance &&
      pJS.interactivity.status == 'mousemove'
    ) {
      /* Draw the line */
      var color_line = pJS.particles.line_linked.color_rgb_line;
      ...
      pJS.canvas.ctx.strokeStyle =
        ...
        color_line.b +
        ',' +
        (pJS.interactivity.line_linked.opacity - dist_mouse / pJS.interactivity.mouse.distance) +
        ')';
      pJS.canvas.ctx.moveTo(p1.x, p1.y);
